...
const myBott = ['u5a9693b5d9a6544575814708a0a1bad0'];//TARO MID LU DISINI SUPAYA BISA PKE COMMAND STAFF

const myBot = ['u5a9693b5d9a6544575814708a0a1bad0'];//TARO MID LU DISINI
...
let { listMember } = await this.searchGroup(seq.to);
     const mentions = await this.mention(listMember);
        ...
        if(txt == 'creator') {
           this._sendMessage(seq, 'My Creator Is Cool\nId Line : http://line.me/ti/p/~sandy_erenjaeger.\n\n-RENDRA-');
           seq.contentType=13;
           seq.contentMetadata = { mid: 'u5a9693b5d9a6544575814708a0a1bad0' };
           this._client.sendMessage(1, seq);
        }
        ...
        if(txt == 'bye') {
           if(!isAdminOrBot(seq.from) || !isStaffOrBot(seq.from)){
          let txt = await this._sendMessage(seq, 'Kami Dari Anak Rendra Mengucapkan Terima Kasih Atas Groupnya Dan Kami Izin Leave~');
